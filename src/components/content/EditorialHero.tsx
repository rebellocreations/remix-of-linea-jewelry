...
const EditorialHero = () => {
  ...
  // Parallax and Siatra-style scroll effects
  ...
  // Siatra effect: Container shrinking and rounding
  const scale = useTransform(scrollY, [0, 500], [1, 0.9]);
  const borderRadius = useTransform(scrollY, [0, 500], ["0rem", "2rem"]);

  // Siatra effect: Text moving up faster and fading out
  ...
  return (
    <section
      ref={heroRef}
      className="relative h-[90vh] md:h-screen min-h-[600px] md:min-h-[700px] w-full overflow-hidden flex items-center justify-center lg:justify-start bg-[#1A1A1A]" // Added black background for the "floating card" effect
    >
      {/* Background Image Container with Shrink & Round effect */}
      <motion.div
      ...
      {/* Content Content - Left Aligned with Fast Parallax and Fade */}
      <motion.div
      ...
      >
        ...
        <motion.button
          ...
          className={`absolute -bottom-20 left-1/2 -translate-x-1/2 text-white/70 hover:text-white transition-colors duration-300 flex flex-col items-center gap-2 ${isLoaded ? "opacity-100" : "opacity-0"
            }`}
