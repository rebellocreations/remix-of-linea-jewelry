...
const BrandStory = () => {
  ...
  return (
    ...
    >
      {/* Text column */}
      <div className="flex flex-col justify-center px-8 lg:px-16 py-24 lg:py-36 order-2 lg:order-1">
        <div className="max-w-md">
          <span
            className={`inline-block text-xs tracking-[0.2em] uppercase text-olive mb-10 transition-all duration-700 ease-premium ${
              isVisible
          ...
          <div className="space-y-7">
            {paragraphs.map((text, index) => (
              <div key={index} className="overflow-hidden">
                <p
                  className={`font-serif text-2xl lg:text-3xl leading-relaxed text-foreground transition-all duration-800 ease-premium ${
                    isVisible
          ...
          <a
            ...
            className={`inline-block mt-14 text-sm tracking-wide text-muted-foreground hover:text-foreground transition-all duration-700 ease-premium editorial-link ${
              isVisible
                ? "opacity-100 translate-y-0"
                : "opacity-0 translate-y-4"
            }`}
      ...
      <div 
        ref={imageRef}
        className="relative h-[60vh] lg:h-auto order-1 lg:order-2 overflow-hidden"
      >
        <div
          className={`absolute inset-0 transition-all duration-1000 ease-premium ${
            imageInView ? "scale-100 blur-0 opacity-100" : "scale-110 blur-md opacity-0"
          }`}
          style={{
